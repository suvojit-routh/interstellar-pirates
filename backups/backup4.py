...
try:
    ...
    rpc.connect()
    continent= str(get_localzone()).split('/')[0]
...
zenoship_buy_surf = pygame.transform.scale(pygame.image.load('graphics/Spaceship/ship4.png'), (125,125)).convert_alpha()
...
buy_zenoship_button = Rect_Button(screen,200,60,forth_item_x,290,'BUY','yellow','red','black')
...
sold_zenoship_button = Rect_Button(screen,200,60,forth_item_x,290,'SOLD OUT','white','red','black',35)
unavailable_xeroship_button = Rect_Button(screen,200,60,third_item_x,290,'UNAVIALABLE','yellow','red','black',30)
unavailable_zenoship_button = Rect_Button(screen,200,60,forth_item_x,290,'UNAVIALABLE','yellow','red','black',30)
...
def shop_menu():
    ...
    first_row_items = [jade_buy_surf,gold_buy_surf,xeroship_buy_surf,zenoship_buy_surf]
    ...
    if game_data["jade"] >= 20000 and game_data["zenoship_owned"] == False:
        if buy_zenoship_button.draw():
            game_data["zenoship_owned"] = True
        buy_zenoship_button.hover('Buy Zenoship by spending 20000 jade','below',20)
    if game_data["zenoship_owned"] == True:
        sold_zenoship_button.draw()
    if game_data["jade"] <= 19999 and game_data["zenoship_owned"] == False :
        unavailable_zenoship_button.draw()
        unavailable_zenoship_button.hover(f'Need 20000 jade (Jade : {game_data["jade"]})','below',20)
...
class Bullets(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        ...
        if game_data["spaceship"] == 0:
            ...
            self.rect = self.image.get_rect()
            self.rect.center = [pos_x, pos_y]
            self.image1 = pygame.image.load('graphics/laser.png').convert_alpha()
            self.rect1 = self.image1.get_rect()
            self.rect1.topleft = [pos_x, pos_y]
            self.image2 = pygame.image.load('graphics/laser.png').convert_alpha()
            self.rect2 = self.image2.get_rect()
            self.rect2.topright = [pos_x, pos_y]

        if game_data["spaceship"] == 1:
            self.image = pygame.image.load('graphics/laser2.png').convert_alpha()
            self.rect = self.image.get_rect()
            self.rect.center = [pos_x, pos_y]
            self.image1 = pygame.image.load('graphics/laser2.png').convert_alpha()
            self.rect1 = self.image1.get_rect()
            self.rect1.topleft = [pos_x, pos_y]
            self.image2 = pygame.image.load('graphics/laser2.png').convert_alpha()
            self.rect2 = self.image2.get_rect()
            self.rect2.topright = [pos_x, pos_y]
        if game_data["spaceship"] == 2:
            self.image = pygame.image.load('graphics/laser3.png').convert_alpha()
            self.rect = self.image.get_rect()
            self.rect.center = [pos_x, pos_y]
            self.image1 = pygame.image.load('graphics/laser3.png').convert_alpha()
            self.rect1 = self.image1.get_rect()
            self.rect1.topleft = [pos_x, pos_y]
            self.image2 = pygame.image.load('graphics/laser3.png').convert_alpha()
            self.rect2 = self.image2.get_rect()
            self.rect2.topright = [pos_x, pos_y]
        if game_data["spaceship"] == 3:
            self.image = pygame.image.load('graphics/laser4.png').convert_alpha()
            self.rect = self.image.get_rect()
            self.rect.center = [pos_x, pos_y]
            self.image1 = pygame.image.load('graphics/laser4.png').convert_alpha()
            self.rect1 = self.image1.get_rect()
            self.rect1.topleft = [pos_x, pos_y]
            self.image2 = pygame.image.load('graphics/laser4.png').convert_alpha()
            self.rect2 = self.image2.get_rect()
            self.rect2.topright = [pos_x, pos_y]
...
def progress_bar(value,data1,data2,pos_y,ratio,text,reward,data_tree="event"):
    ...
    if data_tree == "dailies" and game_data["dailies"]["daily_reset"]:
            # game_data[data_tree][data1][1] = False
...
while run:
    ...
    # INFINITY MODE BULLET CREATION
        if event.type == pygame.MOUSEBUTTONDOWN and state == State.ALIVE:
            if event.button == 3:
                bullets = spaceship.create_bullets()
                bullet_group.add(bullets)
                laser_channel.play(laser_sound)

    # PLANETERY MODE BULLET CREATION
        if event.type == pygame.MOUSEBUTTONDOWN and state == State.PLANETERY_ENTER:
            if event.button == 3:
                bullets = spaceship.create_bullets()
                bullet_group.add(bullets)
                laser_channel.play(laser_sound)
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == K_1 and state == State.ALIVE:
                game_data['spaceship'] = slot_one

            if slot_two!=None:
                if event.key == K_2 and state == State.ALIVE:
                    game_data['spaceship'] = slot_two

            if event.key == K_1 and state == State.PLANETERY_ENTER:
                game_data['spaceship'] = slot_one

            if slot_two!=None:
                if event.key == K_2 and state == State.PLANETERY_ENTER:
                    game_data['spaceship'] = slot_two
            


    # INFINITY MODE XEROSHIP SKILL
            if event.key == K_e and state == State.ALIVE and game_data["spaceship"] == 1 and xeroship_skill_timer == 1000:
                xero_skill = spaceship.skill_2()
                skill2_group.add(xero_skill)
                xeroship_skill_timer = 0

    # PLANETERY MODE MODE XEROSHIP SKILL
            if event.key == K_e and state == State.PLANETERY_ENTER and game_data["spaceship"] == 1 and xeroship_skill_timer == 1000:
                xero_skill = spaceship.skill_2()
                skill2_group.add(xero_skill)
                xeroship_skill_timer = 0

    # INFINITY MODE XEROSHIP ULTIMATE
            if event.key == K_r and state == State.ALIVE and game_data["spaceship"] == 1 and xeroship_ult_timer == 2000:
                projectile1 = spaceship.projectile1()
                projectile_group.add(projectile1)
                xeroship_ult_timer = 0

    # PLANETERY MODE XEROSHIP ULTIMATE
            if event.key == K_r and state == State.PLANETERY_ENTER and game_data["spaceship"] == 1 and xeroship_ult_timer == 2000:
                projectile1 = spaceship.projectile1()
                projectile_group.add(projectile1)
                xeroship_ult_timer = 0

    # INFINITY MODE ZENOSHIP SKILL
            if event.key == K_e and state == State.ALIVE and game_data["spaceship"] == 2  and zenoship_mana>=50:
                sfx_channel.play(teleport_sound)
                spaceship.zenoship_skill()
                zenoship_mana -= 50

    # PLANETERY MODE ZENOSHIP SKILL
            if event.key == K_e and state == State.PLANETERY_ENTER and game_data["spaceship"] == 2 and zenoship_mana>=50:
                sfx_channel.play(teleport_sound)
                spaceship.zenoship_skill()
                zenoship_mana -= 50
    ...
    # INFINITY MODE ZENOSHIP ULT
            if event.key == K_r and state == State.ALIVE and game_data["spaceship"] == 2 and zenoship_mana >= 500:
                lasers = spaceship.creating_laser()
                laser_group.add(lasers)
                zenoship_mana -= 500
    # PLANETERY MODE ZENOSHIP ULT
            if event.key == K_r and state == State.PLANETERY_ENTER and game_data["spaceship"] == 2 and zenoship_mana >= 500:
                lasers = spaceship.creating_laser()
                laser_group.add(lasers)
                zenoship_mana -= 500
    # INFINITY MODE FURRYSHIP SKILL
            if event.key == K_e and state == State.ALIVE and game_data["spaceship"] == 3 and health_orbs >= 100:
                health += 1
                health_orbs = 0

    # PLANETERY MODE FURRYSHIP SKILL
            if event.key == K_e and state == State.PLANETERY_ENTER and game_data["spaceship"] == 3 and health_orbs >= 100:
                health += 1
                health_orbs = 0

     # INFINITY MODE FURRYSHIP ULTIMATE            
            if event.key == K_r and state == State.ALIVE and game_data["spaceship"] == 3 and furryship_ult_timer == 1500:
    ...
    # PLANETERY MODE FURRYSHIP ULTIMATE
            if event.key == K_r and state == State.PLANETERY_ENTER and game_data["spaceship"] == 3 and furryship_ult_timer == 1500:
                meteor_shower = spaceship.furryship_ultimate()
                furry_ult_group.add(meteor_shower)
                furryship_ult_timer = 0
                for _ in range(4):
