...
class Rect_Button:
    ...
    def draw(self, radius=8):
        ...
        text_pos_y = self.image.y + (self.height - text.get_height()) // 2  # <-- HERE FIXED
    ...
    def hover(self, hover_text, side, adjustment):
        ...
        self.below_y = self.image.y + (self.height + adjustment)  # <-- FIXED
        self.right_x = self.pos_x + (self.width + adjustment)
        self.right_y = self.image.y + (self.height - self.text_height) // 2  # <-- FIXED
        self.gap = 5
...
def shop_bottombar(item_icon,item_amount,item_icon2 = None,item_amount2 = None,item_icon3 = None,item_amount3= None , item_icon4 = None,item_amount4= None):
    main_x = 90 + 20
    ...
    if item_icon3 is not None and item_amount3 is not None:
        ...
        fourth_x = third_x + 60 + third_item_amount.get_width() + 20
    if item_icon4 is not None and item_amount4 is not None:
        screen.blit(item_icon4,(fourth_x,screen_height  - 50))
        fourth_item_amount = font.render(f"{item_amount4}",True,"white")
        screen.blit(fourth_item_amount,(fourth_x + 60, screen_height  - 50+5))
...
# Remove scroll_surface fully
scroll_offset = 0
max_scroll = 100

first_row_items = [jade_buy_surf, gold_buy_surf, xeroship_buy_surf, blue_exhaust_buy_surf]

# Create buttons directly on screen
...
def shop_menu():
    global scroll_offset, state, game_data
    ...
    # Draw rectangles + items
    for i in range(4):
        x_position = item_spacing * (i + 1) + shop_rect_width * i
        y_position = 80 + scroll_y
        pygame.draw.rect(screen, 'black', (x_position, y_position, shop_rect_width, shop_rect_height), 0, 8)
        pygame.draw.rect(screen, 'white', (x_position, y_position, shop_rect_width, shop_rect_height), 2, 8)
        image_x = x_position + (200 - 125) // 2
        image_y = y_position + (200 - 125) // 2
    ...
    # Update buttons positions dynamically
    buy_jade_button.image.y = 290 + scroll_y
    unavailable_jade_button.image.y = 290 + scroll_y
    buy_gold_button.image.y = 290 + scroll_y
    unavailable_gold_button.image.y = 290 + scroll_y
    buy_xeroship_button.image.y = 290 + scroll_y
    sold_xeroship_button.image.y = 290 + scroll_y
    unavailable_xeroship_button.image.y = 290 + scroll_y
    buy_blue_exxhaust.image.y = 290 + scroll_y
    sold_blue_exhaust_button.image.y = 290 + scroll_y
    unavailable_blue_exhaust_button.image.y = 290 + scroll_y

 

    # Purchase logic for each button group:
    ...
    # Blue exhaust
    if game_data["gold"] >= 100000 and not game_data["exhaust"]["blue_flame_exhaust"]:
        if buy_blue_exxhaust.draw():
            game_data["exhaust"]["blue_flame_exhaust"] = True
            game_data["gold"] -= 100000
        buy_blue_exxhaust.hover('Buy blue flame exhaust (100000 gold)', 'below', 20)
    elif game_data["exhaust"]["blue_flame_exhaust"]:
        sold_blue_exhaust_button.draw()
    else:
        unavailable_blue_exhaust_button.draw()
        unavailable_blue_exhaust_button.hover(f'Need 100000 gold (Gold : {game_data["gold"]})', 'below', 20)

    header('Shop', 'Black', 'orange', 'white')
    shop_bottombar(jade_icon, game_data["jade"], gold_icon, game_data["gold"])
       # Back & gacha buttons (these stay fixed, no scroll)
    if menu_back_button.draw():
        state = State.MENU
    if gacha_button.draw():
        state = State.GACHA
...
def display_reward(  reward_amount , image_path , return_state): #, finish_state):
    global state, game_data
    screen.fill("black")
    reward_image = pygame.transform.smoothscale(pygame.image.load(image_path) , (200,200)).convert_alpha()
    reward_img_x = (screen.get_width() - reward_image.get_width())//2
    reward_img_y = (screen.get_height() - reward_image.get_height())//2

    font   = pygame.font.Font("font/gacha.ttf", 20)
    amount = font.render(f"x{reward_amount}" , True , "white")
    amount_x = (screen.get_width() - amount.get_width())// 2      
    amount_y = reward_img_y + (reward_image.get_height() + 20)
    screen.blit(reward_image , (reward_img_x , reward_img_y))
    screen.blit(amount ,( amount_x , amount_y))
    if menu_back_button.draw():
        state = return_state
...
def pirates_spaceship_collisions():
    global health,state,research_kills
...
while run:
    ...
    if state == State.MODES:
        ...
        if infinity_button.draw():
            ...
            state = state.SELECTION
        if campaign_button.draw():
            current_state = State.CAMPAIGN
            state = State.CAMPAIGN
        if research_button.draw():
    ...
    if state == State.LOADING:
        if loading_time < 100:
            ...
            if loading_time >= 100:
                loading_time = 0 
                state = current_state  #you can put any condition you want
    if state == State.SHOP:
        shop_menu()
