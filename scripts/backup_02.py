...
def calculate_damage():
    ...
    # Get equipment data for the current ship
    ship_equipment_key = f"{ship_name}_equipment"
    equipment = game_data[ship_equipment_key]

    # Calculate total damage multiplier from equipped weapons
    damage_multiplier = 1
    for item, equipped in equipment.items():
        if equipped:
            damage_multiplier *= equipment_data[item]["boost"][0] or 1

    # Final AA damage
    aa_damage = base_damage * damage_multiplier

    # Skill and Ultimate damage
    skill_damage = None
    ultimate_damage = None

    if ship["skill_multiplier"]:
        skill_damage = base_damage * damage_multiplier * ship["skill_multiplier"]

    if ship["ultimate_multiplier"]:
        ultimate_damage = base_damage * damage_multiplier * ship["ultimate_multiplier"]
...
class Upgrade:
    ...
    def draw_stats(self):
        ...
        stats = [
            ...
            f"Base Damage: {ship['base_damage']}",
            f"AA Damage: {round(aa, 2)}",
            f"Ultimate Damage: {round(ult, 2)}",
            f"Health: {health}",
            f"Speed: {speed}",
        ]
        colors = ["white","yellow","red","purple","green","orangered"]
